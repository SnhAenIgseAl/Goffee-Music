			...
			<div class="main">
				...
				<el-scrollbar>
					<el-table 
						...
						class="table"
					>
						<el-table-column prop="albumId" label="编号" width="90" />
						<el-table-column prop="albumName" label="名称" width="180" />
						<el-table-column prop="albumDscrp" label="简介" show-overflow-tooltip />
						...
						<el-table-column label="操作">
							<template #default="scope">
								<el-button link 
									@click="approveAlbum(scope.$index, scope.row)">
									通过
								</el-button>
...
const getReviewAlbums = async () => {
	try {
		let res = await fetch(`${serverCfg.host}${serverCfg.port}/api/admin/reviewAlbum`, {
...
// 审核通过歌单
const approveAlbum = async (index, albumInfo) => {
	try {
		let res = await fetch(`${serverCfg.host}${serverCfg.port}/api/admin/approve`, {
			method: 'POST',
			headers: { 
				'Content-Type': 'application/json',
				'Authorization': `Bearer ${localStorage.getItem('userToken')}` 
			},
			body: JSON.stringify({albumInfo: albumInfo})
		})
		res = await res.json()
		if (res.success) {
